...
void heap<T, Compare>::heapifyDown(size_t currentIdx)
{
    ...
    size_t theChild = maxPriorityChild(currentIdx);
    if(higherPriority(_elems[theChild], _elems[currentIdx])){
        std::swap(_elems[currentIdx], _elems[theChild]);
        heapifyDown(theChild);
    }
}
...
void heap<T, Compare>::updateElem(const size_t & idx, const T& elem)
{
    ...
    if(idx != root() && higherPriority(elem, _elems[parent(idx)])){
        heapifyUp(idx);
    }
    else {
        heapifyDown(idx);
