	...
	for (int x = 0; x < source.getWidth(); x++)
	{
		for (int y = 0; y < source.getHeight(); y++)
		{
			...
			if (y < source.getHeight() - 1)
				// cout << "X: " << x <<" Y: " << y << " is " << source.isWall(x, y, MazeReader::DOWNWALL) << endl;
...
// 	cs225::PNG* one = maze.drawMaze();
//     one->writeToFile("2.png");
...
void helpSolveMaze(const MazeReader & soln)
{
	...
	cs225::PNG* one = maze.drawMazeWithSolution();
	one->writeToFile("1.png");
...
// 	// cout << "hi" << endl;
// 	// cout << (*actualOutput == solnImage) <<endl;
