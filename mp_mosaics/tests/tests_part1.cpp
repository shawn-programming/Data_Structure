...
TEST_CASE("KDTree::findNearestNeighbor (2D), testing correct path with fence jumping", "[weight=1][part=1]") {
  ...
  action.trigger = true;
  // cout << "Answer: " << tree.findNearestNeighbor(target)[0] << " " << tree.findNearestNeighbor(target)[1] << endl;
  // cout << "target is: " << tree.findNearestNeighbor(target)[0] <<" "<< tree.findNearestNeighbor(target)[1] << " Founded" << endl;
  REQUIRE(tree.findNearestNeighbor(target) == expected);
}
...
TEST_CASE("KDTree::findNearestNeighbor (3D), testing tie-breaking", "[weight=1][part=1]") {
  ...
  action.trigger = true;
  // cout <<"target: "<<tree.findNearestNeighbor(target)[0]<<" "<<tree.findNearestNeighbor(target)[1]<<" "<<tree.findNearestNeighbor(target)[2]<<endl;
  // cout <<"target: "<<tree.findNearestNeighbor(target2)[0]<<" "<<tree.findNearestNeighbor(target2)[1]<<" "<<tree.findNearestNeighbor(target2)[2]<<endl;
  REQUIRE( tree.findNearestNeighbor(target) == expected );
