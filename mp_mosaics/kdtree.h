...
class KDTree
{
  private:
    ...
    struct KDTreeNode
    {
      ...
      KDTreeNode const &operator=(const KDTreeNode& node) {}
    };

  public:
    void clear(KDTreeNode* node);
    void clear();
    void MakeTree(KDTreeNode*& node, vector<Point<Dim>> points, int currDim, int start, int end);
    void assign(KDTreeNode* to, KDTreeNode* from);
    void findNearestR(KDTreeNode* curr, const Point<Dim>& query, Point<Dim>& closest, int currDim, bool& flag) const;
    bool valid(Point<Dim>& curr, const Point<Dim>& query, Point<Dim>& closest, int dim) const;
    /**
