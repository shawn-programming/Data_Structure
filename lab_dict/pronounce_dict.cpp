...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    /* Your code goes here! */
    auto iterWord1 = dict.find(upper1);
    auto iterWord2 = dict.find(upper2);
    if(iterWord1 == dict.end() || iterWord2 == dict.end()) return false;


    auto v1 = (*iterWord1).second;
    auto v2 = (*iterWord2).second;
    if(iterWord1->second == iterWord2->second) return true; 
